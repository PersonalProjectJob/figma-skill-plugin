...
// Restore persisted port preference from clientStorage and push to UI.
(function() {
  figma.clientStorage.getAsync('portPreference')
    .then(function(stored) {
      if (!stored || !stored.preferredPort) return;
      figma.ui.postMessage({ type: 'PORT_PREFERENCE_RESTORED', preference: stored });
      console.log('🌉 [Desktop Bridge] Restored port preference:', stored.preferredPort);
    })
    .catch(function(error) {
      console.warn('🌉 [Desktop Bridge] Could not restore port preference:', error && error.message ? error.message : String(error));
    });
})();
...
figma.ui.onmessage = async (msg) => {
  ...
  // ============================================================================
  // STORE_PORT_PREFERENCE - Save preferred port for this file
  // ============================================================================
  else if (msg.type === 'STORE_PORT_PREFERENCE') {
    try {
      var portPref = { preferredPort: msg.port || null, updatedAt: Date.now() };
      figma.clientStorage.setAsync('portPreference', portPref)
        .then(function() {
          console.log('🌉 [Desktop Bridge] Port preference saved:', portPref.preferredPort);
          figma.ui.postMessage({ type: 'PORT_PREFERENCE_STORED', success: true });
        })
        .catch(function(error) {
          console.warn('🌉 [Desktop Bridge] Could not save port preference:', error && error.message ? error.message : String(error));
          figma.ui.postMessage({ type: 'PORT_PREFERENCE_STORED', success: false });
        });
    } catch (error) {
      console.warn('🌉 [Desktop Bridge] Port preference save error:', error);
    }
  }

  // ============================================================================
  // GET_PORT_PREFERENCE - Read preferred port from clientStorage
  // ============================================================================
  else if (msg.type === 'GET_PORT_PREFERENCE') {
    figma.clientStorage.getAsync('portPreference')
      .then(function(stored) {
        figma.ui.postMessage({ type: 'PORT_PREFERENCE_RESULT', requestId: msg.requestId, success: true, preference: stored || null });
      })
      .catch(function(error) {
        figma.ui.postMessage({ type: 'PORT_PREFERENCE_RESULT', requestId: msg.requestId, success: false, preference: null });
      });
  }
