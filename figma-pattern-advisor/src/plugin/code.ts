...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'INSERT_COMPONENT') {
    ...
    try {
      let instance = null;
      try {
        // Try importing from published library
        const importComponent = await figma.importComponentByKeyAsync(key);
        instance = importComponent.createInstance();
      } catch (e) {
        // Fallback: If it fails (e.g. testing in the same file or unpublished library), try local node
        if (id) {
    ...
    for (const comp of components) {
      ...
      // Generate basic signals from component name words
...
async function initializePlugin() {
  const db = await figma.clientStorage.getAsync('pattern_database');
  const config = await figma.clientStorage.getAsync('llm_config');
  figma.ui.postMessage({ type: 'INIT_DB', database: db || [] });
  if (config) {
    figma.ui.postMessage({ type: 'INIT_LLM_CONFIG', config });
  }
  extractContext();
}
